...
public class ThongTinDatPhongDao implements InterfaceDao<ThongTinDatPhong> {
	...
	public List<ThongTinDatPhong> layDanhSachTheoDK(String maKhachHang) {
		...
		try (Connection conn = JDBCUtil.connect(); PreparedStatement stmt = conn.prepareStatement(sql)) {
			...
			while (rs.next()) {
				ThongTinDatPhong dp = new ThongTinDatPhong(rs.getString("maDatPhong"), rs.getDate("ngayDatPhong"),
						rs.getDate("ngayNhanPhong"), rs.getDate("ngayTraPhong"), rs.getString("ghiChu"),
						rs.getInt("tgSuDung"), rs.getFloat("thanhTien"), rs.getString("trangThai"),
						rs.getDate("ngayHoaDon"), rs.getString("maPhong"), rs.getString("maKhachSan"),
